<html lang="en">
	...
	<body bgcolor="#d3d3d3">
		<div class="container">
            <div class="main">
				<div class="menu">
					<div class="menu_link"><a class="menu_link_clicked" href="./action_login.php">Home</a></div>
			...
			<div class="content" id="content">
				<?php
					...
					while($x = mysqli_fetch_assoc($query)){
						...
						$sql2 = "SELECT * FROM movies WHERE genre = '$genre' ORDER BY release_date DESC";
						...
						foreach(array_combine($images, $names) as $image => $name){
							...
							echo '<button class="movies button" value="'.$name.'"><div class="content_content">';
							if($image){
						...
						if($l > 7 && $l < 12){
							while($l <= 12){
								echo '<div class="content_content" style="cursor: auto;"></div>';
								$l = $l + 1;
								$ok = 1;
							}
						}
...
<script>
    ...
    /*$(function(){
        ...
        //addPage(cookie['movie']);
    });

	function func(){
		document.cookie = "movie=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;";
	}

    function addPage(data){
        document.getElementById('content').style.display = "none";
        document.getElementById('movies_div').style.display = "flex";
        $("#movies_div").html(data);
    }*/
    ...
    document.querySelectorAll('.movies[value]').forEach((button) => {
        k = 0;
        button.addEventListener('click', (event) => {
            $.ajax({
                url:"process_movie.php",
                method:"POST",
                data:{
                    movie_name:button.getAttribute('value')
                },
                dataType:"text",
                success:function(data){
					location.replace('./go_to_movie.php');
                }
            });
        });
	});
...
if(isset($_COOKIE[$cookie_name])) {
    echo "<script>addPage('".$_COOKIE[$cookie_name]."');</script>";
}*/
?>